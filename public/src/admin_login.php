<?php
    ...
    if($_POST){
        $email=$_POST['aemail'];
        $pwd=$_POST['temp_pwd'];
        $sql="SELECT * FROM `admin` WHERE `aemail`='$email' AND `apwd`='$pwd'";
        $result=mysqli_query($dbc,$sql);
        if(mysqli_num_rows($result)==1){
            session_start();
            $sql1=mysqli_query($dbc,"SELECT * FROM `admin` WHERE `aemail`='$email' AND `apwd`='$pwd'");
            ...
            echo "<script>alert('Invalid Administrator Login Credentials!')</script>";
        }
    }
?>
<?php
...
<body>
  ...
  <div class="main">
    ...
    <form class="form1" method="post">
      <input class="un" name="aemail" type="email" align="center" placeholder="email" required>
